...
export const PooButton = ({ emoji, dropping, showHint, onPush }: Props) => {
  ...
  return (
    <div className={`${RARITY_CLASS[rarity]} flex flex-col items-center gap-3`}>
      <button
        type="button"
        onClick={handleClick}
      ...
      >
        ...
        <span
          aria-hidden
          className={`absolute bottom-5 text-sm text-white/60 transition-opacity duration-500 ${showHint && !dropping ? "opacity-100" : "opacity-0"}`}
        >
